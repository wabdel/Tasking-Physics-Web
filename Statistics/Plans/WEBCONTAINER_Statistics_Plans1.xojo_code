...
Begin WebContainer WEBCONTAINER_Statistics_Plans1 Implements WebDataSource
   Compatibility   =   ""
   ControlCount    =   0
   ControlID       =   ""
   Enabled         =   True
   ...
   Begin WebListBox Site_ListBox
      ...
      _mPanelIndex    =   -1
   End
   Begin WEBCONTAINER_Statistics_Plans_PlanTypes Statistics_Plans_PlanTypes
      ControlCount    =   0
      ControlID       =   ""
      Enabled         =   True
      Height          =   560
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Function RowData(RowCount as Integer, RowOffset as Integer, SortColumns as String) As WebListboxRowData()
		  ...
		  + "COUNT(*) as total, " _
		  + "physics_tasking.sites.is_uppercase, " _
		  + "SUM( CASE " _
		  ...
		  While Not rs.AfterLastRow
		    ...
		    Var cellRenderer As New WebListBoxStyleRenderer(s, rs.Column("site").StringValue.Trim.Titlecase)
	...
	#tag Method, Flags = &h21
		Private Function UnsortedPrimaryKeys() As Integer()
		  ...
		  + "ORDER BY physics_tasking.sites.name"
...
#tag Events Site_ListBox
	...
	#tag Event
		Sub Pressed(row As Integer, column As Integer)
		  Statistics_Plans_PlanTypes.site_id = Me.RowTagAt( row)
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events Total_Label
	#tag Event
		Sub Opening()
		  Me.Text = "Total plans = " + Self.RowCount.ToString + " "
		End Sub
	#tag EndEvent
#tag EndEvents
#tag ViewBehavior
