...
Begin WebContainer WEBCONTAINER_Points_old
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   300
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function GET_Random_Planner() As Integer
		  If App.last_database_update <> Latest_UPDATE Then
		    ...
		    Points_Avg_Label.Text = "Average points = " + Format( Points_Average, "0.0")
		    Latest_UPDATE = App.last_database_update
	...
	#tag Method, Flags = &h21
		Private Function points_average() As Double
	...
	#tag Method, Flags = &h21
		Private Sub POPULATE_Planners()
		  ...
		  '
		  'For i As Integer = Planners.LastIndex To 0 Step -1
		  '
		  'If Planners(i).isRetired Then
		  '
		  'Planners.RemoveAt( i)
		  '
		  'End If
		  ...
		  'Next
		  '
		  'For Each item As Physics_Tasking.CLASS_User_Record In Planners
		  '
		  ...
		  '+ "WHERE user_id NOT IN (" + id_list + ") " _
		  ...
		  'While Not rs.AfterLastRow
		  '
		  'Planners.Add( New Physics_Tasking.CLASS_User_Record)
		  'Planners( Planners.LastIndex).id = _
		  ...
		  'Wend
		  '
		  'Planners.Sort( AddressOf Physics_Tasking.SORT_User_Records_points )
	...
	#tag ComputedProperty, Flags = &h21
		#tag Getter
			Get
			  Var sql As String = "SELECT VAR_SAMP(physics_tasking.points.total) As standard_deviation " _
			  + "FROM physics_tasking.points " _
			  + "WHERE is_active = TRUE;"
