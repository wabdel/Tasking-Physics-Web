...
Begin WebContainer WEBCONTAINER_Groups Implements WebDataSource
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   610
   ...
   _mName          =   ""
   _mPanelIndex    =   -1
   Begin WebListBox Groups_ListBox
      ColumnCount     =   1
      ColumnWidths    =   ""
      ControlID       =   ""
      Enabled         =   True
   ...
   Begin WebLabel Groups_Label
      ...
      _mPanelIndex    =   -1
   End
   Begin WebButton Add_Group_Button
...
#tag WindowCode
	#tag Method, Flags = &h21
		Private Function ColumnData() As WebListboxColumnData()
		  ...
		  col.Heading = "Group Name" // the name that appears above the column
	...
	#tag Method, Flags = &h21
		Private Function RowCount() As Integer
		  ...
		  Var sql As String = "SELECT COUNT(*) as c FROM physics_tasking.categories;"
	...
	#tag Method, Flags = &h21
		Private Function RowData(RowCount as Integer, RowOffset as Integer, SortColumns as String) As WebListboxRowData()
		  ...
		  Var sql As String = "SELECT * FROM physics_tasking.categories ORDER BY name;"
		  ...
		  While Not rs.AfterLastRow
		    ...
		    row.PrimaryKey = rs.Column("category_id").IntegerValue
		    row.Tag = rs.Column("category_id").IntegerValue
		    
		    Var cellRenderer As New WebListBoxStyleRenderer(s, rs.Column("name").StringValue.Trim.Titlecase)
...
#tag Events Groups_ListBox
	...
	#tag Event
		Sub Pressed(row as integer, column as integer)
		  ...
		  Var theDialog As New WebDialog_Group_Edit
		  theDialog.group_id = Me.RowTagAt( row)
		  theDialog.Show
...
#tag Events Groups_Label
	...
	#tag EndEvent
#tag EndEvents
#tag Events Add_Group_Button
	#tag Event
		Sub Pressed()
		  Var theDialog As New WebDialog_Group_Add
		  theDialog.Show
		End Sub
	#tag EndEvent
#tag EndEvents
	...
	#tag Event
		Sub Run()
		  ...
		  If App.last_database_update <> Latest_UPDATE Then
		    ...
		    Groups_ListBox.ReloadData
		    Groups_Label.Text = "Groups = " + Groups_ListBox.DataSource.RowCount.ToString
