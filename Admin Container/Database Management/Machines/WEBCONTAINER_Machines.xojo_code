...
Begin WebContainer WEBCONTAINER_Machines Implements WebDataSource
   Compatibility   =   ""
   ControlID       =   ""
   Enabled         =   True
   Height          =   610
   ...
   _mName          =   ""
   _mPanelIndex    =   -1
   Begin WebListBox Machines_ListBox
      ColumnCount     =   1
      ColumnWidths    =   ""
      ControlID       =   ""
      Enabled         =   True
   ...
   Begin WebLabel Machines_Label
      ...
      _mPanelIndex    =   -1
   End
   Begin WebButton Add_Machine_Button
   ...
   Begin WebLabel Info_Label
      ...
      Scope           =   2
      TabIndex        =   3
      Text            =   "Click on a machine to edit."
      TextAlignment   =   0
      TextColor       =   &cFFD47900
      Tooltip         =   ""
      Top             =   552
...
#tag WindowCode
	#tag Method, Flags = &h21
		Private Function ColumnData() As WebListboxColumnData()
		  ...
		  col.Heading = "Machine Name" // the name that appears above the column
	...
	#tag Method, Flags = &h21
		Private Function RowCount() As Integer
		  ...
		  Var sql As String = "SELECT COUNT(*) as c FROM physics_tasking.machines"
	...
	#tag Method, Flags = &h21
		Private Function RowData(RowCount as Integer, RowOffset as Integer, SortColumns as String) As WebListboxRowData()
		  ...
		  Var sql As String = "SELECT * FROM physics_tasking.machines ORDER BY name;"
		  ...
		  While Not rs.AfterLastRow
		    ...
		    row.PrimaryKey = rs.Column("machine_id").IntegerValue
		    row.Tag = rs.Column("machine_id").IntegerValue
	...
	#tag Method, Flags = &h21
		Private Function SortedPrimaryKeys(sortColumns as String) As Integer()
		  ...
		  Var sql As String = "SELECT * FROM physics_tasking.machines"
		  
		  Var rs As RowSet = Physics_Tasking.DB_SELECT_Statement( sql)
		  
		  While Not rs.AfterLastRow
		    keys.Append( rs.Column("machine_id").IntegerValue)
...
#tag Events Machines_ListBox
	...
	#tag Event
		Sub Pressed(row as integer, column as integer)
		  ...
		  Var theDialog As New WebDialog_Machine_Edit
		  theDialog.machine_id = Me.RowTagAt( row)
		  theDialog.Show
...
#tag Events Machines_Label
	...
	#tag EndEvent
#tag EndEvents
#tag Events Add_Machine_Button
	#tag Event
		Sub Pressed()
		  Var theDialog As New WebDialog_Machine_Add
		  theDialog.Show
		End Sub
	#tag EndEvent
#tag EndEvents
	...
	#tag Event
		Sub Run()
		  ...
		  If App.last_database_update <> Latest_UPDATE Then
		    ...
		    Machines_ListBox.ReloadData
		    Machines_Label.Text = "Machines = " + Machines_ListBox.DataSource.RowCount.ToString
