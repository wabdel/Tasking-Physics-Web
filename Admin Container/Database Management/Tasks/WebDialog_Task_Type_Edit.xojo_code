	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Setter
			Set
			  ...
			  + "WHERE task_type_id = " + mtask_type_id.ToString + ";"
			  Var rs As RowSet = Physics_Tasking.DB_SELECT_Statement(sql)
			  
			  Task_Type_Name_TextField.Text = rs.Column("name").StringValue.Trim
			  
			  
			  Weight_TextField.Text = Format(rs.column("weight").DoubleValue, "#0.0#" )
			  Has_Multiplier_Checkbox.Value = rs.Column("has_multiplier").BooleanValue
			  Is_Schedulable_Checkbox.Value = rs.Column("is_schedulable").BooleanValue
			  
			  Instructions_TextArea.Text = rs.Column("instructions").StringValue
			  
			  Task_Group_PopupMenu.SelectRowWithTag( rs.Column("task_group_id").IntegerValue)
		...
		Sub Pressed()
		  ...
		  Try
		    ...
		    If db.Connect Then
		      ...
		      db.Prepare("UPDATE physics_tasking.task_types SET " _
		      + "name = ?, " _
		      + "weight = ?, " _
		      + "has_multiplier = ?, " _
		      + "is_schedulable = ?, " _
		      ...
		      Task_Group_PopupMenu.RowTagAt( Task_Group_PopupMenu.SelectedRowIndex), _
		      task_type_id)
